...
func cmdRollback(ctx context.Context, log *slog.Logger, cfg *config.Config) error {
	...
	addr := strings.Split(cfg.Cutover.CurrentRealIPv6, "/")[0]
	...
	log.Info("rollback: restoring original addresses",
		"v6", cfg.Cutover.CurrentRealIPv6, "v4", cfg.Cutover.CurrentRealIPv4)
	_, _ = sshExec(ctx, host,
		fmt.Sprintf("ip -6 addr add %s dev %s nodad", cfg.Cutover.CurrentRealIPv6, iface), to)
	_, _ = sshExec(ctx, host,
	...
	_ = sender.Send(ctx, cfg.Email.AlertEmail, fmt.Sprintf("%s ROLLBACK COMPLETE", cfg.Email.SubjectPrefix),
