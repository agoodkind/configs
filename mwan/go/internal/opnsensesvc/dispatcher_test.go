...
// that pins the AllowedMethods set on the dispatcher.
...
func TestDispatcherAcksStreamingFragmentBeforeUnmarshal(t *testing.T) {
	// Streaming-only ACKs happen at the wire layer for flow control,
	// before the dispatcher tries to unmarshal the payload. A malformed
	// payload therefore returns from the sender's wait normally; the
	// dispatcher signals the application-level failure with a FlagError
	// response frame on a separate code path.
	srv := newTestServer(t, dispatcherSampleConfig())
	client, stop := startDispatcher(t, srv)
	defer stop()
	...
	if err != nil {
		t.Fatalf("SendStreamMessage err=%v want nil (wire-level ACK fires before payload unmarshal)", err)
	}
}
