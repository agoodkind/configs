...
func cutoverMain() {
	...
	if runErr != nil {
		...
		_ = cutoverSendEmail(cfg, fmt.Sprintf("%s FAILED: %s", subjectPfx, sub),
...
func cmdFull(ctx context.Context, log *slog.Logger, cfg *CutoverConfig) error {
	...
	_ = cutoverSendEmail(cfg, fmt.Sprintf("%s STARTING full cutover (%s)", subjectPfx, mode),
	...
	for _, p := range phases {
		...
		_ = cutoverSendEmail(cfg, fmt.Sprintf("%s Phase: %s STARTING", subjectPfx, p.name),
			fmt.Sprintf("Starting phase %q at %s", p.name, time.Now().Format(time.RFC3339)))

		if err := p.fn(ctx, log, cfg); err != nil {
			_ = cutoverSendEmail(cfg, fmt.Sprintf("%s Phase: %s FAILED", subjectPfx, p.name),
		...
		_ = cutoverSendEmail(cfg, fmt.Sprintf("%s Phase: %s OK", subjectPfx, p.name),
			fmt.Sprintf("Phase %q completed at %s", p.name, time.Now().Format(time.RFC3339)))
	}

	_ = cutoverSendEmail(cfg, fmt.Sprintf("%s CUTOVER COMPLETE", subjectPfx),
