...
(rendered by Ansible from `production.toml.j2` or `suburban-testbed.toml.j2`, or written
literally for lxc-100 and lxc-116). The SMTP2GO API key is injected at runtime via a
...
- Result: lxc-116 and lxc-100 keep their literal (non-`.j2`) `config.toml` files. The
  static toml carries the non-secret email fields. The secret stays out of the toml.
...
- Plan: `mwan/docs/MWAN-132-email-unification-plan.md`.
