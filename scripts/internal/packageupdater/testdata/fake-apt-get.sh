...
# file named by PACKAGE_UPDATER_RECORD. A call carrying the update verb in any
# position waits for the file named by FAKE_APT_GET_UPDATE_GATE, so a test can
# signal the script mid-step. The wait is bounded so a test that fails before
# opening the gate leaves no process polling behind it.
set -euo pipefail
