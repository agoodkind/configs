...
- **Rule (enforced)**: Never infer a value from whether it was set, in any form, on an input variable: not `| default(...)`, not `is defined`, not `.get(key, default)` (a default in disguise), and not `(value | trim) | length` (an "is this set" presence check in disguise). Declare every input value explicitly in the service's group_vars (or `service_mapping.yml` or OpenTofu) and read it bare; a missing value must fail at load time, not silently default. All four forms are allowed only on module or register **output** (the shape of a command result), never on a config input. A sequence size check on a real list is fine; checking whether a scalar is non-empty is not. `when:` is for logic branches between actions, driven by an explicit flag, not defensive programming.
