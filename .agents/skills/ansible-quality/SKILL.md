...
- **Enforcement**: `configs lint` flags every occurrence. The ansible helper runs it before each deploy, the lint path runs it, and pre-commit runs it on staged files. There is no per-line escape hatch and no allowlist; the check grants no exception, so a genuine outside-service case is the author's call to defend, not the check's to allow.
- **Symptoms**:
