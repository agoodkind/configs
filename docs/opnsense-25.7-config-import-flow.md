...
For the **full-restore** GUI path (most relevant for our prod-shaped XML rehearsal):
...
## Practical recommendation for MWAN-127 and MWAN-149

Recommendation: use **direct file replacement + reboot** for our automated rehearsal, but adopt three of the GUI's safety practices:
...
Things to watch on VM 102 during rehearsal:

- The `<version>` field after import. It will likely be bumped by the first migration write that fires during boot. Capture it before and after.
- Per-model migration log lines on first boot (`Migrated OPNsense\Foo\Bar from 1.0.0 to 1.0.1`). MWAN-152 rollback design needs to know whether any model bumped its version.
- The first-boot `set_networking_interfaces_ports` interactive prompt. Our pre-import gate should make this unreachable, but if it fires we know the gate failed.
- The `filter_configure_sync` triple-call (steps 14, 18, 23). Confirm pf rules are stable after the third pass before declaring success.
...
Live reads on VM 102 (`OPNsense 25.7 (amd64)` build `25.7-n271606-9af17f0102ca`, FreeBSD 14.3-RELEASE-p1):
