...
- The OPNsense VM runs `opnsensectl daemon serve`, the FreeBSD daemon from the `opnsensectl` binary that edits the router config over the serial channel. It has no `/etc/mwan/`; its settings are FreeBSD `rc.conf.d` entries.
