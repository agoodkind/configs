...
| vmbrtrunk | Services LAN (OPNsense MANAGEMENT) | VLAN-aware trunk, vids `64 100 200 300`, with a suburban host stub on the untagged LAN. That LAN holds the OPNsense MANAGEMENT interface, the DNS64 LXC, seaweedfs, tack-qa, and VM 950 management. Guest addresses live in [service_mapping.yml](../../ansible/inventory/group_vars/all/service_mapping.yml); the host stub is in [networks.tf](../../opentofu/suburban/networks.tf). "MWAN-140 slice 1". |
